...
gpio_tbl_t gpio_tbl[BODAL_GPIO_MAX_CH] =
{
  {GPIOD, GPIO_PIN_0, GPIO_INPUT, HIGH, HIGH},  // 0. SDCARD_CD
};
...
bool gpioInit(void)
{
  ...
  for (i=0; i<BODAL_GPIO_MAX_CH; i++)
  {
    if (gpio_tbl[i].mode == GPIO_OUTPUT)
    {
...
{
  return (HAL_GPIO_ReadPin(gpio_tbl[channel].port,  \
      gpio_tbl[channel].pin) == GPIO_PIN_SET)?HIGH:LOW;
}
...
void gpioCmdif(void)
{
  ...
  if (cmdifGetParamCnt() == 1)
  {
    if (cmdifHasString("show", 0) == true)
    {
      ...
      {
        for (int i=0; i<BODAL_GPIO_MAX_CH; i++)
        {
